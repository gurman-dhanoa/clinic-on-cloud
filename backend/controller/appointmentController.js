...
exports.getAppointmentDetailsDoctor = catchAsyncError(async(req,res,next)=>{
    const appointment = await Appointment.findById(req.params.id);
    if(!appointment){
        return next(new ErrorHandler("appointment not exist",404));
    }
...
exports.getUserPendingAppointments = catchAsyncError(async (req,res,next)=>{
    ...
    const appointments = await Appointment.find({userId:req.user,status:"pending"});
    res.status(200).json({
        success:true,
        "Total appointments": appointments.length,
        appointments
...
exports.getUserCompletedAppointments = catchAsyncError(async (req,res,next)=>{
    const user = await User.findById(req.user);
    if(!user){
        return next(new ErrorHandler("user not exist",404));
    }

    const appointments = await Appointment.find({userId:req.user,status:"Completed"});
    res.status(200).json({
        success:true,
        "Total appointments": appointments.length,
        appointments
...
exports.deleteAppointment = catchAsyncError(async (req,res,next)=>{
    ...
    await Appointment.findByIdAndUpdate(req.params.id,{status:"cancel"},{
        new: true,
        runValidators: true,
...
exports.getDoctorPendingAppointments = catchAsyncError(async (req, res,next) => {
    const doctor = await Doctor.findById(req.doctor);
    if(!doctor){
        return next(new ErrorHandler("Doctor not exist",404));
    }
    const appointments = await Appointment.find({doctorId:req.doctor,status:"pending"});
    res.status(200).json({
      success: true,
      "Total appointments": appointments.length,
      appointments,
...
exports.getDoctorCompletedAppointments = catchAsyncError(async (req, res,next) => {
    const doctor = await Doctor.findById(req.doctor);
    if(!doctor){
        return next(new ErrorHandler("Doctor not exist",404));
    }
    const appointments = await Appointment.find({doctorId:req.doctor,status:"completed"});
    res.status(200).json({
      success: true,
      "Total appointments": appointments.length,
      appointments,
